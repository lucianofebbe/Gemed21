...
using IP.Services.Usuario.Menu;
...
namespace IP.Services
{
    public class ServicesFactory : IServicesFactory
    {
        ...
        public async Task<MenuService> Create(MenuRequest request, CancellationToken cancellationToken)
        {
            return new MenuService(
                request,
                cancellationToken,
                new LogsFactory(),
                new RepositoryFactory<Domains.Domain.Menu>(),
                new MapperFactory<Domains.Domain.Menu, MenuRequest, MenuResponse>());
